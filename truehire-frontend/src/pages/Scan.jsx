...
function SignalCard({ signal, fired }) {
  const name =
    typeof signal === "string"
      ? signal
      : signal?.label || signal?.name || signal?.title || "Suspicious signal";

  const reason =
    typeof signal === "string"
      ? ""
      : signal?.reason || signal?.description || signal?.message || "";

  const category = typeof signal === "string" ? "" : signal?.category || "";
...
export default function Scan() {
  ...
  const risk = normalizeRisk(
    result?.risk_level ?? result?.risk ?? result?.verdict
  );
  ...
  async function analyzeJob() {
    if (!text.trim()) {
      setError("Please paste a job posting or recruiter message first.");
      return;
    }
    ...
    try {
      ...
      const normalizedResult = {
        ...
        risk_level: normalizeRisk(
          data?.risk_level ?? data?.risk ?? data?.verdict ?? "LOW"
        ),
        ml_probability: Number(data?.ml_probability ?? 0),
        signals_fired: Array.isArray(data?.signals_fired)
          ? data.signals_fired
          : Array.isArray(data?.signals)
          ? data.signals
          ...
          ? data.suspicious_signals
          : [],
        signals_passed: Array.isArray(data?.signals_passed)
          ? data.signals_passed
          : [],
        company_name: data?.company_name ?? data?.company ?? null,
        job_title: data?.job_title ?? data?.jobTitle ?? null,
        salary_claimed:
          data?.salary_claimed ?? data?.salary ?? data?.stipend ?? null,
      ...
      if (err?.message?.includes("Failed to fetch")) {
        setError(
          `Cannot connect to backend server at ${API_URL}. Ensure the backend service is active.`
        );
      } else {
        setError(err?.message || "Unable to analyze this job.");
      }
      setHasAnalyzed(false);
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen bg-[#070b20] text-white">
      ...
      <main className="mx-auto w-full max-w-[1240px] px-5 pb-24 pt-16 sm:px-8 lg:px-10">
        ...
        <section className="mb-10">
          ...
          <p className="mt-4 max-w-3xl text-base leading-7 text-slate-400 sm:text-lg">
            Analyze job postings and recruiter messages for suspicious signals
            before you apply.
          </p>
        </section>
        ...
        <section className="grid gap-6 lg:grid-cols-[minmax(0,1.75fr)_minmax(340px,0.8fr)]">
          ...
          <div className="rounded-2xl border border-white/10 bg-[#11162f]/90 p-5 shadow-2xl sm:p-8">
            <div className="mb-5 flex items-center justify-between">
              ...
              <span className="text-xs text-slate-500">TrueHire Scanner</span>
            </div>
            ...
            <div className="mt-3 flex items-center justify-between text-xs text-slate-600">
              ...
              <span>{loading ? "ANALYZING..." : "TrueHire Scanner"}</span>
            </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-[#11162f]/90 p-6 shadow-2xl sm:p-8">
            <div className="flex min-h-full flex-col items-center justify-center">
              ...
              <div className="mt-7 text-center">
                ...
                <p className="mt-3 text-sm text-slate-500">
                  {firedSignals.length} suspicious signal
                  {firedSignals.length === 1 ? "" : "s"} detected
                </p>
              </div>
          ...
          <section className="mt-7 rounded-2xl border border-white/10 bg-[#11162f]/90 p-6 shadow-2xl sm:p-8">
            ...
            <div className="mb-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
                ...
                <p className="mt-2 font-semibold text-slate-200">
                  {result.salary_claimed || "Not specified"}
                </p>
              </div>
            ...
            <div>
              <div className="mb-4 flex items-center justify-between">
                <h3 className="font-bold text-white">Suspicious signals</h3>
                <span className="rounded-full bg-[#ff4d6d]/10 px-3 py-1 text-xs font-bold text-[#ff4d6d]">
                  {firedSignals.length} flagged
                </span>
              </div>
              ...
              {firedSignals.length > 0 ? (
                <div className="grid gap-3 md:grid-cols-2">
                  {firedSignals.map((signal, index) => (
                    <SignalCard
                      key={
                        signal?.id ?? signal?.name ?? `fired-${index}`
                      }
                      signal={signal}
                      fired={true}
                    />
                ...
                <div className="rounded-xl border border-[#22d36b]/20 bg-[#22d36b]/5 p-5">
                  ...
                  <p className="mt-1 text-sm text-slate-400">
                    The provided job posting did not trigger any configured
                    fraud signals.
                  </p>
                </div>
              )}
            ...
            {passedSignals.length > 0 && (
              <div className="mt-8">
                <div className="mb-4 flex items-center justify-between">
                  <h3 className="font-bold text-white">Signals passed</h3>
                  <span className="rounded-full bg-[#22d36b]/10 px-3 py-1 text-xs font-bold text-[#22d36b]">
                    {passedSignals.length} passed
                  </span>
                </div>
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  {passedSignals.map((signal, index) => (
                    <SignalCard
                      key={
                        signal?.id ?? signal?.name ?? `passed-${index}`
                      }
                      signal={signal}
                      fired={false}
                    />
